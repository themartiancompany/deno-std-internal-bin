...
/** The environment variable used for setting diff context length. */ export const DIFF_CONTEXT_LENGTH = "DIFF_CONTEXT_LENGTH";
/**
 ...
 */ export function getTruncationContextLengthFromEnv() {
  ...
  return Number.isFinite(truncationContextLength) && truncationContextLength >= 0 ? truncationContextLength : null;
}
function getTruncationEnvVar() {
  ...
  if (typeof Deno === "object") {
    ...
    return permissionStatus === "granted" ? Deno.env.get(DIFF_CONTEXT_LENGTH) ?? null : null;
  }
  const nodeEnv = process?.getBuiltinModule?.("node:process")?.env;
  return typeof nodeEnv === "object" ? nodeEnv[DIFF_CONTEXT_LENGTH] ?? null : null;
}
/**
 ...
 */ export function truncateDiff(diffResult, stringDiff, contextLength) {
  ...
  for(let i = 0; i < diffResult.length; ++i){
    ...
    if (result.type === "common") {
      ...
      messages.push(...consolidateCommon(commons, commons.length === i ? "start" : "middle", stringDiff, contextLength));
      commons.length = 0;
      messages.push(result);
    }
  }
  messages.push(...consolidateCommon(commons, "end", stringDiff, contextLength));
  return messages;
}
/**
 ...
 * ```
 */ export function consolidateCommon(commons, location, stringDiff, contextLength) {
  ...
  const indent = location === "start" ? getIndent(after) : location === "end" ? getIndent(before) : commonIndent(before, after);
...
function commonIndent(line1, line2) {
  ...
  return indent1.startsWith(indent2) ? indent2 : indent2.startsWith(indent1) ? indent1 : "";
}
function getIndent(line) {
  return line.match(/^\s+/g)?.[0] ?? "";
}
//# sourceMappingURL=truncate_build_message.js.map